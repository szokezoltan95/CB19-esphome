...
schema_dict = {
    ...
    cv.Optional(CONF_FACTORY_RESET_BUTTON): button.button_schema(CB19FactoryResetButton, entity_category=ENTITY_CATEGORY_CONFIG).extend({cv.Optional(CONF_NAME, default="Factory Reset"): cv.string}),
    cv.Optional(CONF_AUTO_LEARN_BUTTON): button.button_schema(CB19AutoLearnButton, entity_category=ENTITY_CATEGORY_CONFIG).extend({cv.Optional(CONF_NAME, default="Auto Learn"): cv.string}),
    cv.Optional(CONF_REMOTE_LEARN_BUTTON): button.button_schema(CB19RemoteLearnButton, entity_category=ENTITY_CATEGORY_CONFIG).extend({cv.Optional(CONF_NAME, default="Remote Learn"): cv.string}),
    cv.Optional(CONF_CLEAR_REMOTE_LEARN_BUTTON): button.button_schema(CB19ClearRemoteLearnButton, entity_category=ENTITY_CATEGORY_CONFIG).extend({cv.Optional(CONF_NAME, default="Clear Remote Learn"): cv.string}),
